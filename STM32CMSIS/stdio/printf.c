...
/**
 * @brief  Writes a signed long inside the given string, using the provided fill & width
 *         parameters.
 *
 * @param pStr   Storage string.
 * @param fill   Fill character.
 ...
 */
signed int vsnprintf(char *pStr, size_t maxlength, const char *pFormat, va_list ap)
{
    size_t length=maxlength-15;// -len of [truncated] = 11 + size for \r \n \r \0
    char          fill;
    unsigned char width;
    signed int    num = 0;
    signed int    size = 0;
    ...
    while (*pFormat != 0 && size < length) {
        ...
        /* Token delimiter */
            ...
            case 'p':case 'x': num = PutHexa(pStr, fill, width, 0, va_arg(ap, unsigned int)); break;
            case 'P':case 'X': num = PutHexa(pStr, fill, width, 1, va_arg(ap, unsigned int)); break;
    ...
    if (size < length) {
        ...
        *pStr = 0;
        ...
        //truncated !
        *pStr++='[';*pStr++='t';*pStr++='r';*pStr++='u';
        *pStr++='n';*pStr++='c';*pStr++='a';*pStr++='t';
        *pStr++='e';*pStr++='d';*pStr++=']';*pStr++='\r';
        *pStr++='\n';*pStr++='\r';
        *pStr = 0;
    }
...
{
   return vsnprintf(pString, MAX_STRING_SIZE, pFormat, ap);//-2 to let \r\n been embedded eventually
}

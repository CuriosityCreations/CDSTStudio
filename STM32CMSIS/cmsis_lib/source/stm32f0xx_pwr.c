...
@endverbatim
 ...
 /* BESPOON PATCH {{{ */  
static volatile uint8_t PWR_ForbidLowPower=0;
/* }}} BESPOON PATCH */
/**
...
/* BESPOON PATCH {{{ */  
/**
  * @brief  Authorize/Forbid entering in low power mode
  * @note BESPOON PATCH
  * @param  PREVENT: specifies if we can or not enter in low power mode
  *         This parameter can be one of the following values:
  * 		@arg ENABLE: we wont enter in low power mode
  *		@arg DISABLE: we can enter in low power mode
  * @retval None
  */  
void PWR_PreventFromDeepSleep(FunctionalState PREVENT)
{
	PWR_ForbidLowPower = (PREVENT==ENABLE)?1:0;
}
...
/**
  ...
  * 		@arg ENABLE: we 're prevented from entering deep sleep
  *		@arg DISABLE: we can enter in deep sleep
  */
FunctionalState PWR_IsPreventedFromDeepSleep()
{
	return (PWR_ForbidLowPower == 1)?ENABLE:DISABLE;
}
/* }}} BESPOON PATCH */
